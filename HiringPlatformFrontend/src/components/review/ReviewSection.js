...
const ReviewComponent = () => {
  ...
  const calculateReviewStats = (reviewsToCalculate) => {
    ...
    if (reviewsToCalculate.length === 0) {
      ...
      const averageGrade =
        totalReviews > 0 ? (totalGrades / totalReviews).toFixed(2) : 0;
  ...
  const renderDeleteButton = (reviewId) => {
    const hasReplies = reviews.some(
      (reply) => reply.parentReviewId === reviewId,
    );
    return (
  ...
  const renderReviewWithReplies = (review) => {
    const replies = reviews.filter(
      (reply) =>
        reply.parentReviewId && reply.parentReviewId === review.reviewId,
    );
    return (
        ...
        <div>
          {isEditing && review.reviewId === editedReview.reviewId ? (
              ...
              <div className="review-right-content">
                ...
                <ButtonGroup minimal={true} vertical={false}>
                  ...
                  <Button
                    onClick={handleEdit}
                    intent="primary"
                    icon={<Icon icon={"floppy-disk"} size={13} />}
                    minimal
                    className="user-button-for-outline"
                    small
                  >
            ...
            <div>
              <div className="review-content">
                ...
                <div className="review-right-content">
                  ...
                  <ButtonGroup minimal={true} vertical={false}>
                    ...
                    {userIdLogged !== review.userId && (
                      <Button
                        onClick={() => openComplainDialog(review.userEmail)}
                        minimal
                        className="user-button-for-outline"
                        intent={Intent.DANGER}
                        icon={<Icon icon={"issue"} size={13} />}
                        small
                      >
                        {t("report")}
                      </Button>
                    ...
                    {!review.parentReviewId && (
                      <Button
                        onClick={() => handleAddReplyClick(review.reviewId)}
                        minimal
                        intent={Intent.NONE}
                        icon={<Icon icon={"key-enter"} size={13} />}
                        small
                      >
                        {t("reply")}
                      </Button>
              ...
              {showReplyForm &&
                addedReplyReview.parentReviewId === review.reviewId && (
                  <div className="add-review-form">
                    ...
                    <ButtonGroup minimal={true} vertical={false}>
                      <Button
                        onClick={handleResetAddedReplyForm}
                        minimal
                        intent={Intent.NONE}
                        className="user-button-for-outline"
                        icon={<Icon icon={"cross"} size={13} />}
    ...
    <Card className="review-container">
      ...
      <div className="review-first-section">
        <div className="review-header">
          <p className="review-stats-average">
            {reviewStats.averageGrade} {t("out_of")} 5
          </p>
          ...
          <div className="review-progress-info">
            {Object.keys(reviewStats.gradeCounts).map((grade, index) => (
              <div key={index} className="review-grade-count">
                ...
                <ProgressBar
                  className={
                    "review-progress " + getProgressBarClassName(grade)
                  }
                  value={
                    reviewStats.totalReviews === 0
                      ? 0
                      : reviewStats.gradeCounts[grade] /
                        reviewStats.totalReviews
                  }
                  animate={false}
                />
        ...
        <div className="review-form-container">
          {showForm ? (
            <div className="review-form">{renderAddReviewForm()}</div>
          ) : (
            candidate &&
            candidate.candidateId !== "" && (
              <Button
      ...
      <Divider />
      {reviews.map(
        (review) => !review.parentReviewId && renderReviewWithReplies(review),
      )}
      <ComplaintDialog
