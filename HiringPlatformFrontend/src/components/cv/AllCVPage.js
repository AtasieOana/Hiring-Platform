...
const AllCvPage = () => {
  ...
  const addCV = (file, isStandard = true, fileNameUploaded = "") => {
    ...
    let fileName;
    if (isStandard) {
      let nrStandard = data.filter((cv) =>
        cv.cvNameComplete.includes("_Standard_"),
      ).length;
  ...
  const deleteCv = () => {
    CandidateService.deleteCv(cvToDelete.cvId)
      .then((response) => {
        AppToaster.show({
  ...
  return (
    <div>
      ...
      <div className="multiple-cv-background">
        <div className="multiple-cv-title">{t("multiple_cv_title")}</div>
        ...
        <div className="add-cv-options">
          ...
          <table className="bp4-html-table bp4-html-table-bordered cv-table-list">
            <thead>
              <tr>
                <th>
                  <Tooltip
                ...
                <th>
                  <Tooltip>
                    <AnchorButton
                      className="cv-sort-button cv-default-button"
                      minimal={true}
                    >
                      {t("actions")}
                    </AnchorButton>
