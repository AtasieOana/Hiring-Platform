...
const AddJobDialog = ({
  ...
  const STAGE2 =
    "Employer viewed the application (Angajatorul a vizualizat aplicarea)";
  ...
  const citiesPerRegions = useSelector(
    (state: any) => state.address.citiesPerRegions,
  );
  ...
  useEffect(() => {
    ...
    JobService.getAllStages()
      .then((response: any) => {
        ...
        let stage1 = stagesResponse.filter(
          (stage: any) => stage.stageName === STAGE1,
        )[0];
        let stage2 = stagesResponse.filter(
          (stage: any) => stage.stageName === STAGE2,
        )[0];
        let stage3 = stagesResponse.filter(
          (stage: any) => stage.stageName === STAGE3,
        )[0];
  ...
  const handleAddStage = (index: number) => {
    ...
    const availableStages = stages.filter(
      (stage: any) =>
        !selectedStages.some(
          (selected: any) => selected.stage.stageId === stage.stageId,
        ),
    );
  ...
  const handleAddQuestion = () => {
    ...
    questionsCopy.push({
      questionText: "",
      questionNumber: questionsCopy.length,
    });
    setQuestions(questionsCopy);
  };
  ...
  const renderStageItem = (
    stage: any,
    { handleClick, modifiers, _query }: any,
  ): any => {
  ...
  const validateFields = () => {
    ...
    if (
      questions.filter(
        (q: any) => q.questionText.length < 10 || q.questionText.length >= 500,
      ).length > 0
    ) {
      isValid = false;
    }
  ...
  const handleSubmit = () => {
    ...
    if (fieldsValid) {
      ...
      if (employmentRegime === "Stagiu" || employmentRegime === "Internship") {
        ...
        employmentRegime === "Proiect" ||
        employmentRegime === "Project"
      ) {
        employmentRegimeDb = "Proiect";
      } else if (
        employmentRegime === "Contract determinat" ||
        employmentRegime === "Fixed-term Contract"
      ) {
  ...
  if (stages.length > 0) {
    ...
    let filteredStages = stages.filter(
      (stage: any) => !justStages.includes(stage),
    );
    let stageNamesStages = filteredStages.map(
      (stageObj: any) => stageObj.stageName,
    );
    ...
    <MultistepDialog
      ...
      finalButtonProps={{
        ...
        disabled:
          questions.filter(
            (q: any) =>
              q.questionText.length < 10 || q.questionText.length >= 500,
          ).length > 0,
      }}
    >
      <DialogStep
          ...
          <div className="add-job-card">
            ...
            <div className="add-job-general-details">
              <FormGroup
                ...
                intent={
                  jobTitle.length < 5 || jobTitle.length > 500
                    ? "danger"
                    : "none"
                }
                helperText={
              ...
              <FormGroup
                ...
                helperText={
                  jobDescription.length < 100 ? t("job_desc_error") : ""
                }
      ...
      <DialogStep
        ...
        nextButtonProps={{
          disabled:
            !cityName ||
            (cityName && !/^[A-Za-zăâîșțĂÂÎȘȚ\s\-]*$/u.test(cityName)) ||
            !regionName ||
            (regionName && !/^[A-Za-zăâîșțĂÂÎȘȚ\s\-]*$/u.test(regionName)) ||
            false,
          intent: "success",
          text: t("next_step"),
        }}
          ...
          <div className="add-job-card">
            ...
            <div className="add-job-work">
              <div className="add-job-address">
                <FormGroup
                  ...
                  intent={
                    !regionName ||
                    (regionName &&
                      !/^[A-Za-zăâîșțĂÂÎȘȚ\s\-]*$/.test(regionName))
                      ? Intent.DANGER
                      : Intent.NONE
                  }
                  helperText={
                ...
                <FormGroup
                  ...
                  intent={
                    !cityName ||
                    (cityName && !/^[A-Za-zăâîșțĂÂÎȘȚ\s\-]*$/.test(cityName))
                      ? Intent.DANGER
                      : Intent.NONE
                  }
                  helperText={
              ...
              <div className="general-details-select">
                <FormGroup label={t("contract_type")} labelFor="contract-type">
                  <Select
                    items={contracts}
                    fill={true}
                    matchTargetWidth={true}
                ...
                <FormGroup label={t("industry")} labelFor="industry">
                  <Select
                    items={industries}
                    fill={true}
                    matchTargetWidth={true}
                ...
                <FormGroup label={t("work_mode")} labelFor="work-mode">
                  <Select
                    items={works}
                    fill={true}
                    matchTargetWidth={true}
                ...
                <FormGroup label={t("experience")} labelFor="experience">
                  <Select
                ...
                <FormGroup label="" labelFor="empty"></FormGroup>
              </div>
            </div>
          </div>
        }
      ...
      <DialogStep
        ...
        nextButtonProps={{
          ...
          disabled:
            selectedStages.filter(
              (stageObj: any) =>
                stageObj.stage.stageId === OTHERS &&
                stageObj.stage.stageName === "Others (Altele)" &&
                (customStage.length <= 0 || customStage.length >= 200),
            ).length > 0 || selectedStages.length <= 3,
        }}
        backButtonProps={{
          intent: "none",
          text: t("prev_step"),
        ...
        panel={
          <div className="add-job-card">
            ...
            <p className="stages_p">{t("stages_p")}</p>
                    ...
                    {index === selectedStages.length - 2 && index >= 2 ? (
                      <div className="stage-item-select">
                        ...
                        {showInputGroup && (
                          <FormGroup
                            intent={
                              customStage.length <= 0 ||
                              customStage.length >= 200
                                ? "danger"
                                : "none"
                            }
                            helperText={
                  ...
                  <div className="add-stage-buttons">
                    {index === selectedStages.length - 2 && index >= 2 && (
                      <div>
                        <Button
                          intent="warning"
                          className="btn-remove-stage user-button-for-outline"
                          text={t("remove_stage")}
                    ...
                    {index === selectedStages.length - 2 && index >= 1 && (
                      <div>
                        <Button
                          intent="primary"
                          className="btn-add-stage user-button-for-outline"
                          onClick={() => handleAddStage(index)}
                          text={t("add_stage")}
                          disabled={
                            stageValue?.stage?.stageId === "Others" &&
                            (customStage.length <= 0 ||
                              customStage.length >= 200)
                          }
                          small={true}
      ...
      <DialogStep
        ...
        panel={
          <div className="add-job-card">
            ...
            <p className="stages_p">{t("question_p")}</p>
            <div>
              {!showQuestionSection && (
