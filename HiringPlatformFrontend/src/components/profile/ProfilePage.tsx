...
const ProfilePage = () => {
  ...
  const getProfileAsCandidate = () => {
    ...
    setAddress(
      `${profileResponse.street}, ${profileResponse.cityName}, ${t("ro")}`,
    );
    JobService.getNrJobsForEmployer(openedJob.employer.employerId)
      .then((response: any) => {
  ...
  const getProfileAsEmployer = () => {
    ProfileService.getProfile(employer.userDetails.email)
      .then((response: any) => {
        ...
        if (
          profileResponse.regionName === BUCHAREST_RO &&
          i18n.language === "en"
        ) {
          profileResponse.regionName = BUCHAREST_ENG;
          profileResponse.cityName = BUCHAREST_ENG;
        ...
        setIsLoading(false);
        JobService.getNrJobsForEmployer(employer.employerId).then(
          (response: any) => {
            setJobNr(response.data);
          ...
          <div className="profile-left-column">
            <Card className="profile-card">
              <div className="profile-info">
                ...
                <p className="profile-jobs">
                  {t("jobs_available")}: {jobNr}
                </p>
            ...
            <Card className="profile-card">
              <div className="profile-info">
                ...
                <div className="profile-details">
                  <p>
                    {t("email_simple")}: {usedEmployer.userDetails.email}
                  </p>
                  <p>
                    {t("website")}: {profileInfo.site || t("info_not_provided")}
                  </p>
                  <p>
                    {t("phone_number")}:{" "}
                    {profileInfo.phone || t("info_not_provided")}
                  </p>
                </div>
              </div>
            </Card>
            <Card className="profile-card">
              <div className="profile-info">
                ...
                <div className="profile-details">
                  <p>
                    {profileInfo.street}, {profileInfo.zipCode},{" "}
                    {profileInfo.cityName}
                  </p>
                  <p>
                    {t("region")} {profileInfo.regionName}
                  </p>
    ...
    <div>
      ...
      {isLoading ? (
        <div className="central-spinner-div">
          <Spinner className="central-spinner" size={200} />
        </div>
      ) : !isEditing ? (
        renderStaticPage()
      ) : (
