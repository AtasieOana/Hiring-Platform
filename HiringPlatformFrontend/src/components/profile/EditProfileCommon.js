...
const EditProfileCommon = ({
  ...
  const citiesPerRegions = useSelector(
    (state) => state.address.citiesPerRegions,
  );
  ...
  useEffect(() => {
    ...
    let allRegions = regions.filter((r) => r !== BUCHAREST_ENG);
    if (i18n.language === "en") {
      firstRegion = BUCHAREST_ENG;
      allRegions = regions.filter((r) => r !== BUCHAREST_RO);
    }

    if (formData.region === undefined || formData.region === "") {
      setFormData({
        ...formData,
        city: citiesPerRegions[BUCHAREST_RO][0],
        region: firstRegion,
      });
  ...
  useEffect(() => {
    const switchLanguage = () => {
      ...
      const updatedRegionsOption = allRegions.map((region) =>
        region === regionToCheck ? regionToSet : region,
      );
  ...
  const validateForm = () => {
    ...
    if (
      !formData.street ||
      formData.street.length < 10 ||
      !/^[A-Za-zăâîșțĂÂÎȘȚ\d\s.,\-]*$/u.test(formData.street)
    ) {
    ...
    fieldsToValidate.forEach((field) => {
      if (
        !formData[field] ||
        (formData[field] && !/^[A-Za-zăâîșțĂÂÎȘȚ\s\-]*$/u.test(formData[field]))
      ) {
  ...
  const editEmployerProfile = (request) => {
    if (typeof request.imagine === "string") {
      request.imagine = base64ToImage(
        `data:image/jpeg;base64,${request.imagine}`,
      );
    }
    ...
    return (
      <div className="right-column">
        <div className="profile-column-title-desc">
          <span>{t("the_description")}</span>
          {errors.description && (
            <span className="profile-column-desc-error">
              {t("job_desc_error")}
            </span>
          )}
        </div>
