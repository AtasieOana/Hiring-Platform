...
import {
    ...
    Classes, Dialog, DialogBody, DialogFooter, Divider, Drawer, FileInput,
    FormGroup, H3, H5,
    Icon,
    InputGroup,
    Intent, MenuItem,
...
import {dateEn, dateRo, possibleDates} from "../HomePage";
...
const ApplicationPage = () => {
    ...
    const stopRecruitmentProcess = () =>{
        if(reason.length > 0){
            ...
            AppService.refuseApplication(request).then((response) => {
                AppToaster.show({
            ...
            <div className="see-more-app">
                <Card className="see-more-app-card">
                    ...
                    <div className="see-more-app-status">
                        <p>{// Refused
                            currentApp.status === possibleStatus[0] ? t('status_refused') : ""
                        }</p>
                        <p>{// Refused reason
                            currentApp.status === possibleStatus[0] ? '"' + currentApp.refusalReason + '"' : ""
                        }</p>
                        <p>{// In progress
                            currentApp.status === possibleStatus[1] ? t('status_progress') : ""
                        }</p>
                        <p>{// Finished
                            currentApp.status === possibleStatus[2] ? t('status_finished') : ""
                        }</p>
                </div>
                </Card>
                <Card className="see-more-app-card">
                    ...
                    <p className="see-more-app-cv">
                        {t('cv_app_first')} <span className="see-app-cv">{cvShortName}</span>. {t('cv_app_second')}
                    </p>
                    <Button
                        onClick={() => {
                            handleOpenCV({
                ...
                <div className={"right-section"}>
                    <div className={"job-page-buttons"}>
                        <div className="job-page-second-line-buttons">
                            <InputGroup
                                disabled={false}
                                placeholder={t('search_by_candidate')}
                                type="search"
                    ...
                    <div className={paginatingApps.length > 0 ? "jobs-list" : "jobs-non-ideal"}>
                        {paginatingApps.length > 0 ? (
                                            ...
                                            <div className="job-details">
                                                ...
                                                <Button
                                                        ...
                                                        onClick={()=>{
                                                            setCurrentApp(app)
                                                            toggleDrawer()
                                                            console.log(app.stageNr)
                                                            if(app.stageNr === 0 && app.status === possibleStatus[1]) {
                                                                goToSecondStep(app)
                                                            }
                                                        }}
                                                        small
                            ...
                            <NonIdealState
                                ...
                                description={t('no_apps_explanation')}
                            />
                        )}
                    </div>
                    {currentApp && renderTemplateCvDrawer()}
